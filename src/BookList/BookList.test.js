...
describe("BookList Component ", () => {
  ...
  it('should render without crashing', () => {
    ...
    // book list only needs items to be a non-empty array. it just needs to run map.
    // so this is the mvp data model.
    // this allows you to test BookList in isolation -- the rest of the data model traversal is Book's problem.
    sRenderer.render(<BookList bookResults={{items: [2]}} />);

    const result = sRenderer.getRenderOutput();
  ...
  it('renders without crashing', () => {
    ...
    ReactDOM.render(<BookList bookResults={ data } />, div);
    ReactDOM.unmountComponentAtNode(div);
  });  
  it('renders the UI as expected', () => {
    const tree = renderer
        .create(<BookList bookResults={ data } />)
        .toJSON();
